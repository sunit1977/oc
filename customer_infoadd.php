...
$customer_info_add = NULL; // Initialize page object first

class ccustomer_info_add extends ccustomer_info {
	...
	// Page object name
	var $PageObjName = 'customer_info_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "customer_infoview.php")
	...
	function LoadDefaultValues() {
		...
		$this->locality->CurrentValue = NULL;
		$this->locality->OldValue = $this->locality->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// locality
		$this->locality->SetDbValueDef($rsnew, $this->locality->CurrentValue, NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$rsnew['customerid'] = $this->customerid->DbValue;
			}
		} else {
...
// Create page object
if (!isset($customer_info_add)) $customer_info_add = new ccustomer_info_add();

// Page init
$customer_info_add->Page_Init();

// Page main
$customer_info_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$customer_info_add->Page_Render();
?>
...
var CurrentForm = fcustomer_infoadd = new ew_Form("fcustomer_infoadd", "add");

// Validate form
fcustomer_infoadd.Validate = function() {
...
fcustomer_infoadd.Form_CustomValidate = 
...
fcustomer_infoadd.ValidateRequired = true;
<?php } else { ?>
fcustomer_infoadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcustomer_infoadd.Lists["x_state"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fcustomer_infoadd.Lists["x_state"].Options = <?php echo json_encode($customer_info->state->Options()) ?>;
...
<?php if (!$customer_info_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $customer_info_add->ShowPageHeader(); ?>
<?php
$customer_info_add->ShowMessage();
?>
<form name="fcustomer_infoadd" id="fcustomer_infoadd" class="<?php echo $customer_info_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($customer_info_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $customer_info_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $customer_info_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fcustomer_infoadd.Init();
</script>
<?php
$customer_info_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$customer_info_add->Page_Terminate();
?>
