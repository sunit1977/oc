...
$subscribed_to_add = NULL; // Initialize page object first

class csubscribed_to_add extends csubscribed_to {
	...
	// Page object name
	var $PageObjName = 'subscribed_to_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "subscribed_toview.php")
	...
	function LoadDefaultValues() {
		...
		$this->subscriberid->CurrentValue = NULL;
		$this->subscriberid->OldValue = $this->subscriberid->CurrentValue;
		$this->regphone->CurrentValue = NULL;
		$this->regphone->OldValue = $this->regphone->CurrentValue;
		$this->rechargedate->CurrentValue = NULL;
		$this->rechargedate->OldValue = $this->rechargedate->CurrentValue;
		$this->rechargeamount->CurrentValue = NULL;
		$this->rechargeamount->OldValue = $this->rechargeamount->CurrentValue;
		$this->rechargedue->CurrentValue = NULL;
		$this->rechargedue->OldValue = $this->rechargedue->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// servicename
		$this->servicename->SetDbValueDef($rsnew, $this->servicename->CurrentValue, NULL, FALSE);

		// subscriberid
		$this->subscriberid->SetDbValueDef($rsnew, $this->subscriberid->CurrentValue, NULL, FALSE);

		// regphone
		$this->regphone->SetDbValueDef($rsnew, $this->regphone->CurrentValue, NULL, FALSE);

		// rechargedate
		$this->rechargedate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->rechargedate->CurrentValue, 7), NULL, FALSE);

		// rechargeamount
		$this->rechargeamount->SetDbValueDef($rsnew, $this->rechargeamount->CurrentValue, NULL, FALSE);

		// rechargedue
		$this->rechargedue->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->rechargedue->CurrentValue, 7), NULL, FALSE);
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$rsnew['serviceid'] = $this->serviceid->DbValue;
			}
		} else {
...
// Create page object
if (!isset($subscribed_to_add)) $subscribed_to_add = new csubscribed_to_add();

// Page init
$subscribed_to_add->Page_Init();

// Page main
$subscribed_to_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$subscribed_to_add->Page_Render();
?>
...
var CurrentForm = fsubscribed_toadd = new ew_Form("fsubscribed_toadd", "add");

// Validate form
fsubscribed_toadd.Validate = function() {
...
fsubscribed_toadd.Form_CustomValidate = 
...
fsubscribed_toadd.ValidateRequired = true;
<?php } else { ?>
fsubscribed_toadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fsubscribed_toadd.Lists["x_customerid"] = {"LinkField":"x_customerid","Ajax":true,"AutoFill":false,"DisplayFields":["x_customername","x_houseno","x_locality",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"customer_info"};
fsubscribed_toadd.Lists["x_servicename"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_valuename","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"services"};
...
<?php if (!$subscribed_to_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $subscribed_to_add->ShowPageHeader(); ?>
<?php
$subscribed_to_add->ShowMessage();
?>
<form name="fsubscribed_toadd" id="fsubscribed_toadd" class="<?php echo $subscribed_to_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($subscribed_to_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $subscribed_to_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fsubscribed_toadd", "x_rechargedate", 7);
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fsubscribed_toadd", "x_rechargedue", 7);
</script>
<?php } ?>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $subscribed_to_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fsubscribed_toadd.Init();
</script>
<?php
$subscribed_to_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$subscribed_to_add->Page_Terminate();
?>
